...
function changeVolumeAndPlaybackRate() {
  for (let index in myAudios) {
    let audio = myAudios[index];
    audio.volume = window.innerWidth * (1 / screen.availWidth); //볼륨 : 0~1 <- 최대창 크기 시 볼륨 최대로 맞춰놓음
    audio.playbackRate =
...
window.addEventListener("resize", changeVolumeAndPlaybackRate);
